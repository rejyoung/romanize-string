...
from python.language_codes import Code_Language
...
def load_tools(data_group: str):
    vectorizer = joblib.load(MODEL_ASSETS / "vectorizer" / f"ld_{data_group}_vectorizer.joblib")
    model = joblib.load(MODEL_ASSETS / "models" / f"ld_{data_group}_model.joblib")

    return vectorizer, model


def evaluate_input(string: str, data_group: str) -> str:
    vectorizer, model = load_tools(data_group)

    X_base = vectorizer.transform([string])
    X_ext = build_extended_features_block([string], data_group)
