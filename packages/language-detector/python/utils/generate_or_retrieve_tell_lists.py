...
Generate_List_Return: TypeAlias = (
    tuple[dict[str, tuple[str, ...]], tuple[str, ...], tuple[str, ...]]  # (group_map, items, groups)
...
Data_Group_Lists_Return: TypeAlias = dict[str, object]


def generate_or_retrieve_tell_lists(data_group: str) -> Data_Group_Lists_Return:

    ## Load tell lists or generate new tell lists if file is not found
    filepath = TELL_LISTS_DIR / f"ld_{data_group}_tell_lists.joblib"
    filepath.parent.mkdir(parents=True, exist_ok=True)

    try:
        tell_lists = joblib.load(filepath)
    except FileNotFoundError:
        group_tell_chars = tell_character_dict[data_group]
        group_endings = endings_dict.get(data_group)
        group_bigrams = bigrams_dict.get(data_group)
...
def generate_tell_char_lists(group_tell_chars: dict[str, tuple[str, ...]] | None) -> Generate_List_Return:
...
def generate_radical_lists(group_tell_chars: dict[str, tuple[str, ...]] | None) -> Radical_List_Return:
    if not group_tell_chars:
        raise ValueError("Input group_tell_chars cannot be None.")  
...
def generate_endings_lists(group_endings: dict[str, tuple[str, ...]] | None) -> Generate_List_Return:
...
def generate_bigram_lists(group_bigrams: dict[str, tuple[str, ...]] | None) -> Generate_List_Return:
...
endings_dict = {
    "indic": {
        "hi": tuple(sorted(["पन", "ता", "कार", "वादी", "गर", "इया", "इन", "ई", "याँ", "यों"])),
        "mr": tuple(sorted(["णे", "तील", "चा", "ची", "चे", "ला", "ना", "कर", "वाला", "पणा"])),
...
bigrams_dict = {
    "southern_slavic": {
        ...
        "mk": tuple(sorted(["ќе", "ќи", "ќа", "ќу", "ѓе", "ѓи", "ѓа", "ѓу", "ѕв", "ѕд"])),
        "sr": tuple(sorted(["ће", "ћа", "ћу", "ћи", "ђа", "ђе", "ђу", "џв", "џа", "џе"])),
    }
}